...
RSpec.describe RubynCode::Teams::Teammate do
  def build(status: 'idle', parent_agent_id: nil)
    described_class.new(
      id: 't1', name: 'coder', role: 'dev', persona: nil,
      model: nil, status: status, parent_agent_id: parent_agent_id,
  ...
  describe '#root?' do
    it 'returns true when parent_agent_id is nil' do
      expect(build.root?).to be true
    end

    it 'returns false when parent_agent_id is set' do
      expect(build(parent_agent_id: 'parent-1').root?).to be false
    end
  end

  describe '#to_h' do
    it 'returns a hash with all fields including parent_agent_id' do
      mate = build(parent_agent_id: 'parent-1')
      h = mate.to_h
      expect(h).to include(name: 'coder', role: 'dev', status: 'idle', parent_agent_id: 'parent-1')
    end

    it 'includes nil parent_agent_id for root agents' do
      expect(build.to_h[:parent_agent_id]).to be_nil
    end
  end
