...
RSpec.describe RubynCode::Context::Manager, 'compaction pipeline' do
  ...
  describe '#check_compaction!' do
    ...
    it 'tries context collapse before auto-compact when over threshold' do
      ...
      allow(conversation).to receive(:respond_to?).with(:estimated_json_chars).and_return(false)
      allow(conversation).to receive(:respond_to?).with(:refresh_derived_state!).and_return(false)
