...
RSpec.describe RubynCode::IDE::Adapters::ToolOutput do
  ...
  let(:fake_edit_file) do
    Class.new do
      ...
      def preview_content(path:, old_text:, new_text:, replace_all: false) # rubocop:disable Lint/UnusedMethodArgument
        { content: "#{old_text} → #{new_text}#{' (all)' if replace_all}", type: 'modify' }
      end
    end
  end
