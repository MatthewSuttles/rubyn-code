  ...
  module CLI
    ...
    class Setup
      ...
      def run
        ...
        unless gem_wrapper
          $stderr.puts "Error: Could not find the rubyn-code gem wrapper."
          $stderr.puts "Make sure rubyn-code is installed: gem install rubyn-code"
          exit 1
        end

        pinned_ruby = extract_ruby_from_wrapper(gem_wrapper)
        unless pinned_ruby
          $stderr.puts "Error: Could not determine Ruby path from #{gem_wrapper}"
          exit 1
        end
      ...
      def write_launcher(path, gem_wrapper, pinned_ruby)
        File.write(path, <<~BASH)
          #!/usr/bin/env bash
          # Rubyn Code launcher — pinned to Ruby #{File.basename(File.dirname(File.dirname(pinned_ruby)))}
